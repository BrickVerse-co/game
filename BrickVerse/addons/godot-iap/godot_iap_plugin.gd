...
class GodotIapExportPlugin extends EditorExportPlugin:
	...
	const IOS_EXTENSION_TEMPLATE_PATH = "res://addons/godot-iap/bin/godot_iap.gdextension.ios"
	const IOS_EXTENSION_EXPORT_PATH = "res://addons/godot-iap/bin/godot_iap.gdextension"
	...
	func _export_begin(features: PackedStringArray, _is_debug: bool, _path: String, _flags: int) -> void:
		...
		if FileAccess.file_exists(IOS_EXTENSION_TEMPLATE_PATH):
			add_file(
				IOS_EXTENSION_EXPORT_PATH,
				FileAccess.get_file_as_bytes(IOS_EXTENSION_TEMPLATE_PATH),
				false
			)
		else:
			push_warning("[GodotIap] Missing iOS GDExtension descriptor: %s" % IOS_EXTENSION_TEMPLATE_PATH)
