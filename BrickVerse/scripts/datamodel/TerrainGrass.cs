...
public sealed partial class TerrainGrass : Instance
{
	...
	private const string ShaderCode = "shader_type spatial; render_mode cull_disabled; uniform vec4 base_color : source_color; uniform vec4 tip_color : source_color; uniform float height_variation=0.35; uniform float wind_strength=0.28; uniform float wind_speed=1.5; varying float blade_y; varying vec4 blade_tint; void vertex(){ blade_y=UV.y; blade_tint=INSTANCE_COLOR; float h=(1.0+(INSTANCE_CUSTOM.x-0.5)*height_variation)*INSTANCE_CUSTOM.y; VERTEX.y*=h; VERTEX.xz*=INSTANCE_CUSTOM.z; float wave=sin(TIME*wind_speed+INSTANCE_CUSTOM.x*19.0); VERTEX.x+=wave*wind_strength*UV.y*UV.y; } void fragment(){ vec4 gradient=mix(base_color,tip_color,blade_y)*blade_tint; ALBEDO=gradient.rgb; ALPHA=gradient.a; ROUGHNESS=0.9; }";
}
