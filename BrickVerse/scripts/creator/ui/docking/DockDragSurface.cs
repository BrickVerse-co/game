...
public sealed partial class DockDragSurface : Control
{
	...
	private void SetHover(DockRegion region, Vector2 globalPosition)
	{
		...
		Rect2 nextPreview = new(
			globalPreviewOrigin - GetGlobalRect().Position,
			regionLocalRect.Size
		);

		if (_hoverRegion == region && _previewRect == nextPreview)
			return;
		_hoverRegion = region;
	...
	{
		if (_hoverRegion == null && _previewRect == default)
			return;
		_hoverRegion = null;
		_previewRect = default;
		QueueRedraw();
