    ...
    public string? Handle(string message)
    {
        ...
        try
        {
            ...
            switch (method.GetString())
            {
                case "initialize":
                    _initialized = true;
                    result = new { protocolVersion = "2025-06-18", capabilities = new { tools = new { listChanged = false } },
                        serverInfo = new { name = "brickverse-creator", version = "1.0.0" } };
                    break;
                case "ping": result = new { }; break;
                case "tools/list":
                    if (!_initialized) return Error(id, -32002, "Initialize first.");
                    result = new { tools = ForgeToolCatalog.Definitions.Where(t => t.Function.Name != "run_luau")
