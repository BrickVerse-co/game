...
public static class ForgeModelCatalog
{
	...
	[
		new("forge-free", "Forge Free", ForgeProviderKind.ForgeFree, "BrickVerse-hosted model for Astro Basic+ members. Heavily rate-limited.", true, true),
		new("gpt-5.2", "GPT-5.2", ForgeProviderKind.OpenAI, "OpenAI flagship coding and agent model.", true, true),
		new("gpt-5.2-mini", "GPT-5.2 mini", ForgeProviderKind.OpenAI, "Faster and lower-cost OpenAI model.", true),
		new("gpt-4.1", "GPT-4.1", ForgeProviderKind.OpenAI, "General-purpose OpenAI model with tool support.", true),
		new("claude-opus-4-6", "Claude Opus 4.6", ForgeProviderKind.Anthropic, "Anthropic's highest-capability model.", true, true),
		new("claude-sonnet-4-6", "Claude Sonnet 4.6", ForgeProviderKind.Anthropic, "Balanced Claude model for coding and iteration.", true),
		new("claude-haiku-4-5", "Claude Haiku 4.5", ForgeProviderKind.Anthropic, "Fast Claude model for lightweight requests.", true),
		...
		new("gemini-2.5-pro", "Gemini 2.5 Pro", ForgeProviderKind.Google, "Google model for complex coding and large context.", true, true),
		new("gemini-2.5-flash", "Gemini 2.5 Flash", ForgeProviderKind.Google, "Fast Google model for interactive work.", true),
		new("openrouter/auto", "OpenRouter Auto", ForgeProviderKind.OpenRouter, "Let OpenRouter route the request.", true),
		new("custom", "Custom model ID…", ForgeProviderKind.Compatible, "Any OpenAI-compatible local or hosted model.", true),
	];

	public static IEnumerable<ForgeModelDefinition> ForProvider(ForgeProviderKind provider) => Models.Where(model => model.Provider == provider);
	public static ForgeModelDefinition? Find(string id) => Models.FirstOrDefault(model => model.Id.Equals(id, StringComparison.OrdinalIgnoreCase));
}
...
public sealed class ForgeProviderSettings
{
	...
	public bool RequiresApiKey => Provider is not ForgeProviderKind.ForgeFree and not ForgeProviderKind.Compatible;

	public string GetProviderLabel() => Provider switch
	{
...
internal static class ForgeProviderSettingsStore
{
	...
	public static ForgeProviderSettings Load() { try { return FileAccess.FileExists(SettingsPath) ? JsonSerializer.Deserialize(FileAccess.GetFileAsString(SettingsPath), ForgeJsonContext.Default.ForgeProviderSettings) ?? new() : new(); } catch (Exception ex) { BV.PrintErr("Forge settings load failed: ", ex.Message); return new(); } }
	public static void Save(ForgeProviderSettings settings) { try { using FileAccess file = FileAccess.Open(SettingsPath, FileAccess.ModeFlags.Write); file.StoreString(JsonSerializer.Serialize(settings.CloneForSave(), ForgeJsonContext.Default.ForgeProviderSettings)); } catch (Exception ex) { BV.PrintErr("Forge settings save failed: ", ex.Message); } }
}

internal static class ForgeConversationStore
{
	private const string Path = "user://creator/forge_chats.json";
	public static List<ForgeConversation> Load() { try { return FileAccess.FileExists(Path) ? JsonSerializer.Deserialize(FileAccess.GetFileAsString(Path), ForgeJsonContext.Default.ListForgeConversation) ?? [] : []; } catch { return []; } }
	public static void Save(List<ForgeConversation> chats) { using FileAccess file = FileAccess.Open(Path, FileAccess.ModeFlags.Write); file.StoreString(JsonSerializer.Serialize(chats, ForgeJsonContext.Default.ListForgeConversation)); }
}

public sealed class ForgeChatRequest
{
	[JsonPropertyName("model")] public string Model { get; set; } = string.Empty;
	[JsonPropertyName("messages")] public List<ForgeChatMessage> Messages { get; set; } = [];
	[JsonPropertyName("tools")][JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] public List<ForgeChatToolDefinition>? Tools { get; set; }
	[JsonPropertyName("tool_choice")][JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] public string? ToolChoice { get; set; }
	[JsonPropertyName("temperature")][JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] public float? Temperature { get; set; }
	[JsonPropertyName("chat_id")][JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] public string? ChatId { get; set; }
}
...
public sealed class ForgeChatMessage
{
	...
	[JsonPropertyName("tool_calls")][JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)] public List<ForgeChatToolCall>? ToolCalls { get; set; }
	...
	public ForgeChatMessage Clone() => new() { Role = Role, Content = Content, ToolCallId = ToolCallId, Name = Name, ToolCalls = ToolCalls?.ConvertAll(static call => call.Clone()) };
}

public sealed class ForgeChatToolDefinition { [JsonPropertyName("type")] public string Type { get; set; } = "function"; [JsonPropertyName("function")] public ForgeChatToolDefinitionFunction Function { get; set; } = new(); }
...
public sealed class ForgeDeleteInstanceArgs { [JsonPropertyName("path")] public string Path { get; set; } = string.Empty; }
public sealed class ForgeCreateInstanceArgs { [JsonPropertyName("class_name")] public string ClassName { get; set; } = string.Empty; [JsonPropertyName("parent_path")] public string? ParentPath { get; set; } [JsonPropertyName("name")] public string? Name { get; set; } [JsonPropertyName("properties")] public JsonElement Properties { get; set; } }

public sealed class ForgeRunLuauArgs
{
	[JsonPropertyName("source")] public string Source { get; set; } = string.Empty;
	[JsonPropertyName("compatibility")] public bool Compatibility { get; set; }
