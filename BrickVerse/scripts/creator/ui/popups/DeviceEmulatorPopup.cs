...
public sealed partial class DeviceEmulatorPopup : Window
{
	...
	private MessageRuntimeDeviceEmulation State() => new()
	{
		...
		LeftX = (float)_leftX.Value, LeftY = (float)_leftY.Value,
		RightX = (float)_rightX.Value, RightY = (float)_rightY.Value,
		PrimaryButton = _primary.ButtonPressed, SecondaryButton = _secondary.ButtonPressed,
		LeftTrigger = _leftTrigger.ButtonPressed, RightTrigger = _rightTrigger.ButtonPressed,
		HeadYaw = Mathf.DegToRad((float)_headYaw.Value), HeadHeight = (float)_headHeight.Value,
